...
#endif
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"Finding id for ts: %lld (%d syncpts)\n", (long long int)searchts, num_syncpoints);
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk1: %d %d\n", lo, hi);
#endif
    int retval = (hi > lo) ? hi+1 : lo+1; // the value from our local list
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk2: %d %d\n", lo, hi);
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk3: %d %d | %d %d - %d\n", lo, hi, splo, sphi, retval);
#endif
	if (sphi > splo) splo = sphi;
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk4: %d %d | %d %d - %d\n", lo, hi, splo, sphi, retval);
#endif
	if (syncpoints[splo].ts1 == searchts) {
#if AROObjectSynchronizer_C_debugVerbosity >= 2
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk5a: %d %d | %d %d - %d\n", lo, hi, splo, sphi, retval);
#endif
		if (syncpoints[splo].id1 > retval) retval = syncpoints[splo].id1;
#if AROObjectSynchronizer_C_debugVerbosity >= 2
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk6a: %d %d | %d %d - %d\n", lo, hi, splo, sphi, retval);
#endif
	} else if (splo > 0) {
#if AROObjectSynchronizer_C_debugVerbosity >= 2
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk5b: %d %d | %d %d - %d\n", lo, hi, splo, sphi, retval);
#endif
		if (syncpoints[splo-1].id1+1 > retval) retval = syncpoints[splo-1].id1+1;
#if AROObjectSynchronizer_C_debugVerbosity >= 2
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk6b: %d %d | %d %d - %d\n", lo, hi, splo, sphi, retval);
#endif
	} else retval = 1;
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tchk7: %d %d | %d %d - %d\n", lo, hi, splo, sphi, retval);
#endif
    return retval;
}
...
#endif
    AROLog(logDEBUG4,0,"Sync_C","FREEA - %d\n", ugetmalloccounter());
    if (op->syncPoints != NULL)
        AOc_free(op->syncPoints);
    AROLog(logDEBUG4,0,"Sync_C","FREEB - %d\n", ugetmalloccounter());
    AOc_free(op);
    AROLog(logDEBUG4,0,"Sync_C","FREEC - %d\n", ugetmalloccounter());
}

void AROObjectSynchronizerInf_setDebugInfo(AROObjectSynchronizerInf *op, int debugLevel_, const char *debugLabel_) {
	AROLog(logDEBUG4,0,"Sync_C","Updating Sync_C debug info: %d %s\n",
...
void AROObjectSynchronizerInf_sweepToAnchor(AROObjectSynchronizerInf *syncState,
	...
	for (i = 0; i <= syncState->numSyncPoints; i++) {
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
        if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts1 i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
        if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"chk: %d %d - %lld\n", lo, hi, (long long int)sypts[i].ts1);
        if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts2 i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
                if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts2a i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
				if (hi+1 > sypts[i].id1) sypts[i].id1 = hi+1;
#if AROObjectSynchronizer_C_debugVerbosity >= 1
                if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts2b i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
				if ((hi - lo + 1) > sypts[i].id2) { sypts[i].id2 = hi - lo + 1;
					tsreq[*numreq] = sypts[i].ts1; (*numreq)++; }
#if AROObjectSynchronizer_C_debugVerbosity >= 1
                if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts2c i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
				if (i > 0) if ((sypts[i].id1 - sypts[i].id2) <= sypts[i-1].id1)
					sypts[i].id1 = sypts[i-1].id1+sypts[i].id2;
#if AROObjectSynchronizer_C_debugVerbosity >= 1
                if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts2d i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
                
#if AROObjectSynchronizer_C_debugVerbosity >= 1
				if ((sypts[i].id1 != tmppt.id1) || (sypts[i].id2 != tmppt.id2))
					if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tupdating pt_hi %d : (%d %lld) (%d %lld) : %d %d %lld %lld --> %d %d %lld %lld\n", i,
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
				if ((sypts[i].id1 != tmppt.id1) || (sypts[i].id2 != tmppt.id2))
					if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tsingle entry at %d : (%d %lld) (%d %lld) : %d %d %lld %lld --> %d %d %lld %lld\n", i,
                                                lo, (long long int)*(ts + lo*stride), hi, (long long int)*(ts + hi*stride),
                                                tmppt.id1, tmppt.id2, (long long int)tmppt.ts1, (long long int)tmppt.ts2,
                                                sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2
                                                );
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
            if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts3 i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
            if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts4 i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
            
#if AROObjectSynchronizer_C_debugVerbosity >= 1
			if ((sypts[i].id1 != tmppt.id1) || (sypts[i].id2 != tmppt.id2))
				if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tupdating pt_lo %d : (%d %lld) : %d %d %lld %lld --> %d %d %lld %lld\n", i,
		...
		if (lastloc >= 0) {
#if AROObjectSynchronizer_C_debugVerbosity >= 1
            if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts5 i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
			if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tbr-3 the previous one we had in our list was %d at %d\n", lastbnd, lastloc);
			if ((newbnd + newoff - lastbnd) > (sypts[i].id1 - sypts[lastloc].id1))
				if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tupdating pt_span %d : bnd %d loc %d : (%d %lld) (%d %lld) : %d %d %lld %lld --> %d %d %lld %lld\n",
                                            i, lastbnd, lastloc,
                                            lo, (long long int)*(ts + lo*stride), hi, (long long int)*(ts + hi*stride),
                                            tmppt.id1, tmppt.id2, (long long int)tmppt.ts1, (long long int)tmppt.ts2,
                                            sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
            if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\t--chksypts6 i: %d - %d %08x %d %d %lld %lld\n", i, sypts[i].res, sypts[i].hash, sypts[i].id1, sypts[i].id2, (long long int)sypts[i].ts1, (long long int)sypts[i].ts2);
#endif
		}
#if AROObjectSynchronizer_C_debugVerbosity >= 1
		if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tupdate last bnd/loc/off %d / %d / %d --> %d / %d / %d\n",
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
        if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tsplo = %d sphi = %d\n", splo, sphi);
#endif
		if (sphi >= splo) { int i;
            ...
            //#if AROObjectSynchronizer_C_debugVerbosity >= 1
            //if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tDoes not exist in syncpoints\n");
            //#endif
...
int AROObjectSynchronizerInf_processSyncPoint(AROObjectSynchronizerInf *syncState, SyncPoint *proc, int numts, uint64_t *ts, int stride, uint64_t *tsreq, int *numreq_ret, SyncPoint *newsyncpoints, int *num_newsyncpoints_ret) {
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
	if (numts <= 0) AROLog(logDEBUG4,0,debugLabel,"We have no entries - nothing to process in AROObjectSynchronizerInf_processSyncPoint\n");
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
    if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Processing sync point\n\t%d %08x %d %d %lld %lld\n\tfront: %d %lld back: %d %lld\n\n",
...
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"Working with list of %d entries:\n", numts);
	for (i = 0; i < numts; i++) if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\t%d : %lld\n", i+1, (long long int)(*(ts + i*stride)));
#endif
    
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"----------------- sync_C points at start -----------------\n");
	for (i = 0; i <= syncState->numSyncPoints; i++)
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\t%d : %d %d %lld %lld\n", i, syncState->syncPoints[i].id1, syncState->syncPoints[i].id2, (long long int)syncState->syncPoints[i].ts1, (long long int)syncState->syncPoints[i].ts2);
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"--------------------------------------------------------\n");
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
	if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Sweeping %d syncpoints to update:\n", syncState->numSyncPoints);
#endif
	AROObjectSynchronizerInf_sweepToAnchor(syncState, ts, numts, stride, tsreq, &numreq);
	
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"----------------- sync points at mid1 -----------------\n");
	for (i = 0; i <= syncState->numSyncPoints; i++)
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\t%d : %d %d %lld %lld\n", i, syncState->syncPoints[i].id1, syncState->syncPoints[i].id2, (long long int)syncState->syncPoints[i].ts1, (long long int)syncState->syncPoints[i].ts2);
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"--------------------------------------------------------\n");
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
	if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Updated anchor points to sync point\n\t%d %08x %d %d %lld %lld\n\tfront: %d %lld back: %d %lld\n\n",
                                proc->res, proc->hash, proc->id1, proc->id2, (long long int)proc->ts1, (long long int)proc->ts2,
                                AROObjectSynchronizerInf_findIDForTimestamp(proc->ts1, syncState->numSyncPoints, syncState->syncPoints, numts, ts, stride),
                                (long long int)AROObjectSynchronizerInf_findTimestampForID(proc->id1, syncState->numSyncPoints, syncState->syncPoints, numts, ts, stride),
                                AROObjectSynchronizerInf_findIDForTimestamp(proc->ts2, syncState->numSyncPoints, syncState->syncPoints, numts, ts, stride),
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
	if (rqhi < rqlo) { if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"rq %d : %lld\n", numreq, (long long int)tsreq[numreq-1]); }
	else { if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"found %lld in local list at %d %d (%lld %lld)!\n", (long long int)proc->ts1,
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
		if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Sweepflags: %d %d - resweeping %d syncpoints to account for information from proc\n", sweeplo, sweephi, syncState->numSyncPoints);
#endif
		AROObjectSynchronizerInf_sweepToAnchor(syncState, ts, numts, stride, tsreq, &numreq);
		if (syncState->numSyncPoints > 0) {
			if (sweeplo >= 0) {
				if (sweeplo >= syncState->numSyncPoints) sweeplo = syncState->numSyncPoints-1;
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
		if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Start of global picture is floating: %d %lld over %d\n",
	...
	if (flag > 0) { syncdflag = 1;
#if AROObjectSynchronizer_C_debugVerbosity >= 1
		if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"This range is globally sync'd - flag %d!!\n", flag);
#endif
		flag = 0;
		...
		if (flag > 0) {
#if AROObjectSynchronizer_C_debugVerbosity >= 1
			if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tabout to split -- %d %d %d %08x\n", proc->id1,proc->id2,proc->res,proc->hash);
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
			if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"\tsplitting (flag %d) range %d %d with res %d at %d %lld\n",
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
		if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Processing this range - front %d %lld back %d %lld\n", idforts1, (long long int)tsforid1, idforts2, (long long int)tsforid2);
#endif
        
#if AROObjectSynchronizer_C_debugVerbosity >= 2
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"----------------- sync points at mid2 -----------------\n");
		for (i = 0; i <= syncState->numSyncPoints; i++)
			if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\t%d : %d %d %lld %lld\n", i, syncState->syncPoints[i].id1, syncState->syncPoints[i].id2, (long long int)syncState->syncPoints[i].ts1, (long long int)syncState->syncPoints[i].ts2);
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"--------------------------------------------------------\n");
#endif
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
		if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Span of interest: %lld -- %lld\n", (long long int)rqfts, (long long int)rqbts);
#endif
		...
		for (i = 0; i < 4; i++) {
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
			if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"i: %d inspoint: %d %lld\n", i, inspoint.id1, (long long int)inspoint.ts2);
#endif
#if AROObjectSynchronizer_C_debugVerbosity >= 2
			...
			if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\tinsloc for %s = %d : %d %lld / %d %d %lld %lld %08x / %d %d %lld %lld %08x \n",
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
		if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"tsPOI: (%d) ", numtsPOI);
        for (j = 0; j < numtsPOI; j++) AROLog(logDEBUG4,0,debugLabel," %lld", (long long int)tsPOI[j]); AROLog(logDEBUG4,0,debugLabel,"\n");
#endif
        
#if AROObjectSynchronizer_C_debugVerbosity >= 1
		if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Re-sweeping %d syncpoints to update before generating sync requests\n", syncState->numSyncPoints);
#endif
		AROObjectSynchronizerInf_sweepToAnchor(syncState, ts, numts, stride, tsreq, &numreq);
    ...
    //#if AROObjectSynchronizer_C_debugVerbosity >= 1
    //	AROLog(logDEBUG4,0,"Sync_C","Final sweep to look for anchored but incorrect sub-resolution syncpoints - %d req\n", numreq);
    //#endif
    //	for (i = 0; i < syncState->numSyncPoints; i++) {
...
#if AROObjectSynchronizer_C_debugVerbosity >= 2
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"----------------- sync points at end -------------------\n");
	for (i = 0; i <= syncState->numSyncPoints; i++)
	...
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"----------------- ts reqs at end -----------------------\n");
	for (i = 0; i < numreq; i++)
		if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\t%d : %lld\n", i, (long long int)tsreq[i]);
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"--------------------------------------------------------\n");
    
	if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"----------------- sync reqs at end -----------------\n");
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
	if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"End of AROObjectSynchronizerInf_processSyncPoint\n");
#endif
...
void AROObjectSynchronizerInf_resolveLocalAlignment(
	...
	if (numts <= 0) {
		AROLog(logDEBUG4,0,debugLabel,"We have no entries - nothing to process in AROObjectSynchronizerInf_resolveLocalAlignment\n");
		if (syncState->numSyncPoints > 0)
			AROLog(logDEBUG4,0,debugLabel,"Generating requests for entries known about by syncPoints\n");
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
	if (debugLevel >= 1) AROLog(logDEBUG4,0,debugLabel,"Checking local alignment --\n");
    //	AROLog(logDEBUG4,0,"Sync_C","Sweeping %d syncpoints to update before checking local alignment\n", syncState->numSyncPoints);
#endif
    //	AROObjectSynchronizerInf_sweepToAnchor(syncState, ts, numts, stride, tsreq, &numreq);
...
#if AROObjectSynchronizer_C_debugVerbosity >= 1
        if (debugLevel >= 1) {
            if (i == 0) AROLog(logDEBUG4,0,debugLabel,"chk i: %d %lld | %d %d | x %d %d %d\n", i, (long long int)syncState->syncPoints[i].ts1, lo, hi, gid2, lid1, lid2);
            else AROLog(logDEBUG4,0,debugLabel,"chk i: %d %lld | %d %d | %d %d %d %d\n", i, (long long int)syncState->syncPoints[i].ts1, lo, hi, gid1, gid2, lid1, lid2);
        }
#endif
		if ((i > 0) && (lid1 > 0) && (lid2 > 0)) {
#if AROObjectSynchronizer_C_debugVerbosity >= 2
			if (debugLevel >= 2) AROLog(logDEBUG4,0,debugLabel,"\t%d : %d %d : %d %d : %lld %lld\n", i, gid1, gid2, lid1, lid2,
...
int AROObjectSynchronizerInf_checkAllSyncPoints(AROObjectSynchronizerInf *syncState, int numts, uint64_t *ts, int stride) {
	...
	for (i = 0; i <= syncState->numSyncPoints; i++) {
		...
		if ((syncState->syncPoints + i)->id1 > numts) flag = -3;
            ...
            AROLog(logERROR, 1, "Sync", "%d", (syncState->syncPoints + i)->id1);
            flag = -3;
        }
		else {
