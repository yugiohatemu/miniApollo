...
void Application::resume(){
    ...
    t_app_sync.expires_from_now(boost::posix_time::seconds(1));
...
void Application::processMessage(Packet p){
    ...
    switch(p.flag){
        ...
        case BB_MERGE_ACTION: //BB->SYNC
            ...
            break;
        case BB_MERGE_HEADER: //might be BB->BB ->notify of SyncPoint
            strand.dispatch(boost::bind(&Application::bb_mergeHeader, this, &p.content.raw_header));
            break;
        default:
            break;
    }
    ...
    if ((msgSyncPoint.id1 == 0) && (msgSyncPoint.id2 == 0)) {
        ...
        if ((msgSyncPoint.ts1 == 0ll) && (msgSyncPoint.ts2 == 0ll)) { //0,0,0,0 global picture
            ...
            for (; lo <= hi; lo++) {
                ...
                for (unsigned int i = 0; i < peer_list.size(); i++) {
                    if (i!=pid) {
                        Packet packet; packet.flag = APP_MERGE_ACTION; packet.content.ts = ts;
                        peer_list[i]->enqueue(boost::protect(boost::bind(&Application::processMessage, peer_list[i]->application,packet)));
                    }
                }
            }
...
void Application::bb_mergeHeader(Raw_Header_C * raw_header){
...
///////////////////////////////////////////////////////////////
//TODO: add another timer for that
...
void Application::bb_periodicSync(const boost::system::error_code &error){
    //TODO: fill in this first
}
...
void Application::good_peer_first(){
    ...
    for (unsigned int i = 0; i < peer_list.size(); i++) {
        if (i!=pid) {
            Packet packet; packet.flag = BB_MERGE_HEADER; packet.content.raw_header = *(header->raw_header);
            peer_list[i]->enqueue(boost::protect(boost::bind(&Application::processMessage, peer_list[i]->application,packet)));
        }
    }
