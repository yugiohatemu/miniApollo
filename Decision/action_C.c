...
void free_actionList(ActionList_C* ac_list){
    ...
    if (ac_list->header_list) {
        for (unsigned int i = 0; i < ac_list->header_count; i++) {
            if (ac_list->header_list[i].bb) {
                free_backBundle(ac_list->header_list[i].bb);
...
//TODO: encoporate merging into it.
...
void add_header_to_actionList(ActionList_C * ac_list,  Header_C * header){
    ac_list->header_list[ac_list->header_count] = *header;
